...
def spvs_coarse(data, config):
    ...
    def out_bound_mask(pt, w, h):
        return tf.math.logical_or(tf.math.logical_or((pt[..., 0] < 0),(pt[..., 0] >= w)), tf.math.logical_or((pt[..., 1] < 0),(pt[..., 1] >= h)))
  
    idx_n1 = tf.where(out_bound_mask(w_pt0_c_round, w1, h1))
    nearest_index1 = tf.tensor_scatter_nd_update(nearest_index1,idx_n1, tf.zeros(idx_n1.shape[0],dtype=tf.int64))
    ...
    idx_n0 = tf.where(out_bound_mask(w_pt1_c_round, w1, h1))
    nearest_index0 = tf.tensor_scatter_nd_update(nearest_index0,idx_n0, tf.zeros(idx_n0.shape[0],dtype=tf.int64))
    ...
    loop_back = tf.stack([tf.gather_nd(nearest_index0,indices=[[_b,idx] for idx in tf.cast(_i,tf.int32)])
                        for _b, _i in enumerate(nearest_index1)], axis=0)# (N, L)
    ...
    correct_0to1 = tf.cast(loop_back,tf.int32) == tf.repeat(tf.range(h0*w0)[None],N,axis=0) 
    # correct_0to1[:, 0] = False  # ignore the top-left corner
    corr_indexes = tf.stack([tf.range(0,N),tf.zeros(N,tf.int32)],axis=1)
    correct_0to1 = tf.tensor_scatter_nd_update(correct_0to1,corr_indexes, tf.zeros(corr_indexes.shape[0],dtype=tf.bool))
    ...
    # j_ids = tf.convert_to_tensor(nearest_index1.numpy()[b_ids.numpy(),i_ids.numpy()],dtype=tf.int64)
    j_ids = tf.gather_nd(tf.cast(nearest_index1,tf.int64),indices=[[b,i] for b,i in zip(b_ids,i_ids) ])

    ids_as_index = tf.stack([b_ids,i_ids,j_ids],axis=1)
    conf_matrix_gt = tf.tensor_scatter_nd_update(conf_matrix_gt,ids_as_index, tf.ones(ids_as_index.shape[0],dtype=tf.float32))
...
def spvs_fine(data, config):
    ...
    # 3. compute gt
    scale = scale * tf.gather(data['scale1'],indices=data['b_ids']) if 'scale0' in data else scale
    # scale = scale * data['scale1'].numpy()[b_ids.numpy()] if 'scale0' in data else scale
    
    # `expec_f_gt` might exceed the window, i.e. abs(*) > 1, which would be filtered later
    expec_f_gt = (tf.gather_nd(w_pt0_i,indices=[[b,i] for b,i in zip(b_ids,i_ids) ]) - tf.gather_nd(pt1_i,indices=[[b,j] for b,j in zip(b_ids,j_ids) ])) / scale / radius
