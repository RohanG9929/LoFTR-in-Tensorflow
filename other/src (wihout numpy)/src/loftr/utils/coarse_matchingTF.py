...
def mask_border_with_padding(m, bd, v, p_m0, p_m1):
    ...
    for b_idx, (h0, w0, h1, w1) in enumerate(zip(h0s, w0s, h1s, w1s)):
        np_m = np_m[b_idx, h0 - bd:].assign(v)
        np_m = np_m[b_idx, :, w0 - bd:].assign(v)
        np_m = np_m[b_idx, :, :, h1 - bd:].assign(v)
        np_m = np_m[b_idx, :, :, :, w1 - bd:].assign(v)
...
class CoarseMatching(tf.keras.Model):
    ...
    def get_coarse_match(self, conf_matrix, data, training):
        ...
        if training:
            ...
            b_ids, i_ids, j_ids, mconf = map(lambda x, y: tf.concat([tf.cast(tf.gather(x,indices=pred_indices),tf.double), tf.cast(tf.gather(y,indices=gt_pad_indices),tf.double)],axis=0),
                                         *zip([b_ids, data['spv_b_ids']], [i_ids, data['spv_i_ids']],[j_ids, data['spv_j_ids']], [mconf, mconf_gt]))
        ...
        scale0 = scale * tf.gather(data['scale0'],indices=b_ids) if 'scale0' in data else scale # scale0 = scale * data['scale0'].numpy()[b_ids.numpy()] if 'scale0' in data else scale
        scale1 = scale * tf.gather(data['scale1'],indices=b_ids) if 'scale1' in data else scale# scale1 = scale * data['scale1'].numpy()[b_ids.numpy()] if 'scale1' in data else scale

        mkpts0_c = tf.cast(tf.stack([i_ids % data['hw0_c'][1], i_ids // data['hw0_c'][1]],axis=1),tf.float32)*scale0
        mkpts1_c = tf.cast(tf.stack([j_ids % data['hw1_c'][1], j_ids // data['hw1_c'][1]],axis=1),tf.float32)*scale1
