...
class FineMatching(tf.keras.Model):
    ...
    def get_fine_match(self, coords_normed, data):
        ...
        scale1 = scale * tf.gather(data['scale1'],indices=data['b_ids']) if 'scale0' in data else scale

        # scale1 = scale * data['scale1'].numpy()[data['b_ids']] if 'scale0' in data else scale

        mkpts1_f = data['mkpts1_c'] + (tf.cast(coords_normed,tf.float32) * (W // 2) * scale1)[:len(data['mconf'])]
