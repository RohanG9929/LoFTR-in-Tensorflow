...
def import_scannet(npz_path, sens_folders_path, intrinsics_path, score_thresh, no_sample_pairs_per_scene, batch_size, no_scenes):
    ...
    scene_nos = np.random.randint(len(scenes), size=no_scenes)
    ctr = 0
    for pair_info_filename_path in glob.iglob(npz_path+'*'+'.npz'): #ITERATE FOR ALL NPZ FILES IN SCANNET_INDICES
        if ctr in scene_nos:
